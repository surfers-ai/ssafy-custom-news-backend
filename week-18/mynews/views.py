...
from mynews.serializers.dto.news_search_request_serializer import (
    NewsSearchRequestSerializer,
)
from mynews.serializers.dto.news_search_response_serializer import (
...
from mynews.serializers.dto.dashboard_response_serializer import (
...
class NewsListView(APIView):
    def get(self, request: Request, *args, **kwargs):
        ...
        if not request.user.is_authenticated:
            articles, total_count = Article.get_article_list(
                category, page, limit, sort_by
            )
            ...
            if sort_by == "recommend":
                articles, total_count = Article.get_recommendation_article_list(
                    request.user.id, category, page, limit
                )
            else:
                articles, total_count = Article.get_article_list(
                    category, page, limit, sort_by
...
class ArticleView(APIView):
    def get(self, request: Request, article_id: int) -> JsonResponse:
        ...
        try:
            ...
            return SUCCESS_RESPONSE(
                "호출 성공", ArticleResponseSerializer(article).data
            )
...
class WriteArticleView(APIView):
    def post(self, request: Request) -> JsonResponse:
        ...
        try:
            ...
            return SUCCESS_RESPONSE(
                "기사가 성공적으로 생성되었습니다.", ArticleSerializer(article).data
            )
...
class DashboardView(APIView):
    def get(self, request: Request) -> JsonResponse:
        ...
        number_of_written_articles = Article.get_number_of_written_articles(
            request.user.id
        )
        ...
        return SUCCESS_RESPONSE(
            "유저의 취향을 시각화한 대시보드입니다.", serializer.data
        )
...
class LikeArticleView(APIView):
    ...
    def like_found_response(self):
        return Response(
            {"message": "좋아요를 누른 기사입니다.", "is_liked": True},
